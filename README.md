...
```
1. SELECT NPRM  →  Choose a proposed rule or upload your own PDF
2. REVIEW       →  Explore sections, agency proposals, and questions they want answered
3. INTERVIEW    →  AI asks 4-5 targeted questions about your experience
4. DRAFT        →  AI writes a formatted comment with citations and page references
...
│   ├── UploadStep.tsx             # NPRM selection cards
│   ├── ReviewStep.tsx             # Section expander with proposals/reasoning/questions
│   ├── InterviewStep.tsx          # Conversational Q&A with progress
│   ├── DraftStep.tsx              # Markdown comment viewer, edit, copy, download
│   └── SubmitStep.tsx             # User info form + Regulations.gov submission
├── lib/
│   ├── types.ts                   # TypeScript interfaces
│   ├── llm.ts                     # LLM integration (questions + drafting)
│   └── regulations.ts             # Regulations.gov API client
└── data/
...
- [ ] Real-time regulations.gov docket search
